...
- [ ] Add `.env.example` with all required keys (see SETUP.md)
- [ ] Create `.github/workflows/ci.yml` — runs lint, unit tests, E2E
- [ ] Create `.gitignore` (include `.env.local`, `.env.*.local`)
...
### Tasks
- [ ] Create `lib/firebase.ts` — initialize Firebase app (client-side)
- [ ] Create `lib/auth.ts` — `signInWithGoogle()`, `signOut()` helpers
- [ ] Create `lib/firestore.ts` — typed Firestore helpers
- [ ] Create `lib/types.ts` — all shared TypeScript types (see ARCHITECTURE.md)
- [ ] Create `hooks/useAuth.ts` — React context + hook for auth state
- [ ] Create `app/providers.tsx` — wrap app in AuthProvider
- [ ] Create `components/AuthGuard.tsx` — redirects unauthenticated users
- [ ] Create `app/login/page.tsx` — Google Sign-In button page
- [ ] Create `app/onboarding/page.tsx` — display name form (shown once, on first login)
- [ ] Firestore rule: users can only write to their own `users/<uid>` doc
...
### Tasks
- [ ] Create `lib/posts.ts` — CRUD helpers for posts collection
- [ ] Create `components/PostCard.tsx` — displays a single post (author, timestamp, content, reply count)
- [ ] Create `components/PostComposer.tsx` — textarea + submit button to create a post. Supports **Markdown**.
- [ ] Create `components/Feed.tsx` — real-time list of PostCards
- [ ] Update `app/page.tsx` — render Feed + PostComposer (auth-guarded)
- [ ] Post content: supports Markdown (rendered with `react-markdown`)
- [ ] Optimistic UI: post appears immediately on submit
...
### Tasks
- [ ] Create `components/PostActions.tsx` — Edit / Delete dropdown (shown only to OP or admin)
- [ ] Create `components/EditPostModal.tsx` — inline edit form with markdown preview
- [ ] Create `components/DeleteConfirmDialog.tsx` — confirmation before delete
- [ ] Add `edited: true` + `updatedAt` timestamp on edit
- [ ] Show "(edited)" label on PostCard if `edited === true`
- [ ] Wire up delete: removes post doc + all replies subcollection docs (use a Cloud Function OR batch delete — prefer batch on client for Vercel free tier, with a warning comment about scale)
